window.addEventListener('DOMContentLoaded', () => {
  ...
  // // если нет необходимости в шаблонизации - для создания одного элемента используется функция, а не конструктор
  // // getResource('http://localhost:3000/menu')
  ...
  btnNext.addEventListener('click', () => {
    if(offset == createNum(sliderWidth) * (slide.length - 1)) {
      offset = 0;
    } else {
      offset += createNum(sliderWidth);
    }
  ...
  btnPrev.addEventListener('click', () => {
    if(offset == 0) {
      offset = createNum(sliderWidth) * (slide.length - 1);
    } else {
      offset -= createNum(sliderWidth);
    }
    slidesField.style.transform = `translateX(-${offset}px)`;
  ...
  dots.forEach(dot => {
    dot.addEventListener('click', (e) => {
      ...
      slideIndex = slideId;
      offset = createNum(sliderWidth) * (slideId - 1);
      slidesField.style.transform = `translateX(-${offset}px)`;
      setCurrentSlide();
      setActiveDot();
    });
  ...
  function getDinamicValues(selector) {
  ...
  getDinamicValues('#height');
  getDinamicValues('#weight');
  getDinamicValues('#age');
});